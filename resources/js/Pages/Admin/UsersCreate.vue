...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800">Criar Usuário</h2>
        </template>
        <div class="p-6 bg-white rounded shadow">
            <form @submit.prevent="createUser">
                ...
                <div class="mb-4">
                    ...
                    <input v-model="form.password" type="password" class="border rounded w-full" />
                    <div v-if="form.errors.password" class="text-red-600">{{ form.errors.password }}</div>
                </div>

                <div class="mb-4">
                    <label>Confirm Password</label>
                    <input v-model="form.password_confirmation" type="password" class="border rounded w-full" />
                </div>
